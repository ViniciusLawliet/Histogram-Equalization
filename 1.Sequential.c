...
// Function to convert to grayscale (in-place)
void convert_to_grayscale_inplace(pixel *matrix, int height, int width) {
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            uint8_t gray_val = (uint8_t)(val + 0.5); // rounding to nearest
...
// Function to perform histogram equalization (in-place)
void equalize_histogram(pixel *matrix, int height, int width) {
    ...
    uint8_t map[256];
    for (int i = 0; i < 256; i++) {
        map[i] = (uint8_t)((cdf[i] * 255.0 / total_pixels) + 0.5); // max 255.5?
    }
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <input.bmp> <output.bmp> <mask_size>\n", argv[0]);
        return 1;
    }
    ...
    pixel *working = allocate_matrix(height, width); // 2 matrix
    ...
    clock_gettime(CLOCK_MONOTONIC, &ts_start); // Linux <time.h>
